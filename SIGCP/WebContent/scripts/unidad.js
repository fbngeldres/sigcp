...
function filtrar(idFormulario) {
	...
	if (indice == 4) {
		valor = document.formularioFiltro.valorSociedad.value;
	}
	if (valor == "") {
		if (indice == 1) {
		...
		if (indice == 3) {
			alert("Debe especificar el Nombre de la Unidad");
		}
		if (indice == 4) {
			alert("Debe especificar la Sociedad");
		}
		return false;
	}
...
function resetValores() {
	...
	document.getElementById("filtroSociedad").value = "";
}
