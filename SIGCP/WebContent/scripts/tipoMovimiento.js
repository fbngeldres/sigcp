...
function filtrar(idFormulario) {
	var indice;
	if (document.formularioFiltro.filtro != undefined) {
		indice = document.formularioFiltro.filtro.selectedIndex;
	}
	var valor;
	if ((indice == 1) || (indice == 2) || (indice == 3)) {
		valor = document.formularioFiltro.valor.value;
	} else if (indice == 4) {
		valor = document.formularioFiltro.valorClasificacion.value;
	}
	if (valor == "") {
		if (indice == 1) {
		...
		if (indice == 3) {
			alert("Debe especificar el Nombre del Tipo de Movimiento");
		}
		if (indice == 4) {
			alert("Debe especificar una Clasificaci\xf3n");
		}
		return false;
	}
...
function actualizarFiltro(idFormulario, nameCampoValor) {
	...
	if (indice == 4) {
		mostrarFiltroClasificacion();
	}
}
function resetValores() {
	document.getElementById("valor").value = "";
	document.getElementById("valorClasificacion").value = "";
}
function ocultarFiltros() {
	document.getElementById("filtroText").style.display = "none";
	document.getElementById("labelValor").style.display = "none";
	document.getElementById("filtroClasificacion").style.display = "none";
}
...
function mostrarFiltroClasificacion() {
	document.getElementById("filtroText").style.display = "none";
	document.getElementById("labelValor").style.display = "none";
	document.getElementById("filtroClasificacion").style.display = "";
}
