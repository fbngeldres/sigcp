...
public class NotificacionDiariaLogic implements ConstantesMensajePresentacion,
		ConstantesMensajeAplicacion, ConstantesLogicaNegocio,
		NotificacionDiariaLogicFacade, ConstantesSap {
	...
	private static final String ESTADO_APROBADO = ManejadorPropiedades
			.obtenerPropiedadPorClave(ESTADONOTIFICACION_APROBADO);
	private static final String ESTADO_CERRADO = ManejadorPropiedades
			.obtenerPropiedadPorClave(ESTADONOTIFICACION_CERRADO);
	...
	private String TIPO_DOCUMENTO_MATERIAL_INGRESO = ManejadorPropiedades
			.obtenerPropiedadPorClave(TIPODOCUMENTO_INGRESO);
	private String TIPO_DOC_MAT_SALIDA = ManejadorPropiedades
			.obtenerPropiedadPorClave(TIPODOCUMENTO_SALIDA);
	...
	private String ERROR_TIPO_MOV_RFC = ManejadorPropiedades
			.obtenerPropiedadPorClave(CODIGO_SAP_TIPO_MOV_NO_EXISTE);
	...
	private String INGRESO_MERCADERIA = ManejadorPropiedades
			.obtenerPropiedadPorClave(CODIGO_SAP_INGRESO_MERCADERIA);
	...
	public NotificacionDiariaLogic() {
		...
		NotificacionDiariaLogic.beandozerFactory = BeandozerFactoryImpl
				.getInstance();
	 ...
	 */
	public NotificacionDiariaBean obtenerNotificacionDiaria(Long codigo)
		...
		try {
			notificacionDiariaBean = beanFactory
					.transformarNotificacionDiaria(NotificacionDiariaQuerier
							.getById(codigo));
	...
	public NotificacionDiariaBean obtenerUltimaNotificacionDiaria(
			NotificacionDiariaBean notificacion) throws LogicaException {
		...
		try {
			...
			notificacionDiariaBean = beanFactory
					.transformarNotificacionDiaria(NotificacionDiariaQuerier
							.obtenerUltimaNotificacionDiaria(notificacion
	...
	public NotificacionDiariaBean obtenerUltimaNotificacionDiariaAprobada(
			NotificacionDiariaBean notificacion) throws LogicaException {
			...
			notificacionDiariaBean = beanFactory
					.transformarNotificacionDiaria(NotificacionDiariaQuerier
							.obtenerUltimaNotificacionDiariaAprobada(
									notificacion.getLineaNegocio().getCodigo(),
									fechaInicio, fechaFin));
	...
	public NotificacionDiariaBean obtenerPrimeraNotificacionDiariaAprobada(
			NotificacionDiariaBean notificacion) throws LogicaException {
		...
		try {
			...
			notificacionDiariaBean = beanFactory
					.transformarNotificacionDiaria(NotificacionDiariaQuerier
							.obtenerPrimeraNotificacionDiariaAprobada(
									notificacion.getLineaNegocio().getCodigo(),
									fechaInicio, fechaFin));
	...
	private static void aprobar(Integer[] id) throws NumberFormatException,
			LogicaException {
		...
		for (int i = 0; i < id.length; i++) {
			...
			NotificacionDiariaBean notificacionDiaria = notificacionDiariaLogic
					.obtenerNotificacionDiaria(idNotif);
			System.out.println("AGREGO " + notificacionDiaria.getCodigo() + "_"
					+ notificacionDiaria.getFechaNotificacionDiaria());
			notificacionesAprobar.add(notificacionDiaria);
		}
		Collections.sort(notificacionesAprobar,
				new Comparator<NotificacionDiariaBean>() {

					public int compare(NotificacionDiariaBean o1,
							NotificacionDiariaBean o2) {
						int compareFechas = o1.getFechaNotificacionDiaria()
								.compareTo(o2.getFechaNotificacionDiaria());
		...
		for (NotificacionDiariaBean notificacion : notificacionesAprobar) {
			System.out.println("APROBANDO NOTIFICACION "
					+ notificacion.getFechaNotificacionDiaria());
	 ...
	 */
	public Notificaciondiaria obtenerNotificacionDiariaDataObject(Long codigo)
			throws LogicaException {
	 ...
	 */
	public List<NotificacionDiariaBean> obtenerNotificacionesDiariasPorFiltros(
		...
		try {
			notificacionDiariaList = (List<NotificacionDiariaBean>) beandozerFactory
					.tranformarLista(NotificacionDiariaQuerier
							.obtenerNotificacionesDiariasPorFiltros(
									codigoLineaNegocio, codigoEstado,
									fechaInicio, fechaFin),
							NotificacionDiariaBean.class);
		} catch (AplicacionException e) {
			mensajeError = ManejadorPropiedades
					.obtenerPropiedadPorClave(ERROR_FALLA_CONSULTA_LISTA);
			throw new LogicaException(mensajeError, e);
		} finally {
			if (session != null)
				PersistenciaFactory.closeSession(session);
	...
	public List<NotificacionDiariaBean> obtenerNotificacionesDiariasEstadoPorFiltros(
			Long codigoSociedad, Long codigoUnidad, Long codigoLineaNegocio,
			Long codigoEstado, Date fechaInicio, Date fechaFin)
			throws LogicaException {
		...
		try {
			notificacionDiariaList = beanFactory
					.transformarListaNotificacionDiariaSimple(NotificacionDiariaQuerier
							.obtenerNotificacionesDiariasEstadoPorFiltros(
									codigoLineaNegocio, codigoEstado,
									fechaInicio, fechaFin));
	 ...
	 */
	public NotificaciondiariaDTO obtenerNotificacionDiariaDTO(
			Long codigoLineaNegocio, Long codigoTableroControl,
		...
		Notificaciondiaria notificaciondiaria = this
				.obtenerNotificacionDiariaDO(codigoLineaNegocio,
		...
		try {
			notificaciondiariaDTO
					.setPkCodigoNotificaciondiaria(notificaciondiaria
							.getPkCodigoNotificaciondiaria());
			notificaciondiariaDTO
					.setPkCodigoEstadonotificacion(notificaciondiaria
							.getEstadonotificacion()
							.getPkCodigoEstadonotificacion());
			notificaciondiariaDTO.setPkCodigoLineanegocio(codigoLineaNegocio);
			notificaciondiariaDTO
					.setPkCodigoUsuarioRegistra(notificaciondiaria
							.getUsuarioByFkCodigoUsuarioRegistra()
							.getPkCodigoUsuario());
			notificaciondiariaDTO
					.setPkCodigoTablerocontrol(codigoTableroControl);
	 ...
	 * java.util.Date)
	 */
	public Notificaciondiaria obtenerNotificacionDiariaDO(
			Long codigoLineaNegocio, Long codigoTableroControl,
			Date fechaRegistro) throws LogicaException {
		/**
		...
		try {
			notificacion = NotificacionDiariaQuerier.obtenerNotificacionDiaria(
					codigoLineaNegocio, codigoTableroControl, fechaRegistro);
		} catch (ElementoNoEncontradoException e) {
			mensajeError = ManejadorPropiedades
					.obtenerPropiedadPorClave(MAESTROS_REGISTRO_NO_ENCONTRADO);
			throw new LogicaException(mensajeError, e);
	...
	public void actualizarEstadoNotificacion(
			NotificacionDiariaBean notificacionDiariaBean, Long codigoEstado)
			throws LogicaException {
		Transaction tx = null;
		Session session = null;
		...
		try {
			...
			Notificaciondiaria notificacion = NotificacionDiariaQuerier
					.getById(notificacionDiariaBean.getCodigo());

			DateFormat df = new SimpleDateFormat(
					FechaUtil.PATRON_FECHA_APLICACION);
			String fechaStr = df.format(notificacion
					.getFechaNotificaciondiaria());

			try {
				Long codigoEstadoAprobado = new Long(
						ManejadorPropiedades
								.obtenerPropiedadPorClave(CODIGO_ESTADONOTIFICACION_APROBADO));
				notificacion.setFechaNotificaciondiaria(df.parse(fechaStr));
				if (codigoEstado.longValue() == codigoEstadoAprobado
						.longValue()) {
					notificacion.setFechaAprobacionNotificaciondi(Calendar
							.getInstance().getTime());
					Usuario usuarioAprueba = new Usuario();
					usuarioAprueba
							.setPkCodigoUsuario(usuarioSesion.getCodigo());
					notificacion
			...
			Estadonotificacion estado = EstadoNotificacionQuerier
					.getById(codigoEstado);
			...
			mensajeError = ManejadorPropiedades
					.obtenerPropiedadPorClave(MAESTROS_REGISTRO_RELACIONADO);
			PersistenciaFactory.rollbackTransaction(tx);
			throw new LogicaException(mensajeError, e);
		} finally {
			PersistenciaFactory.closeSession(session);
	 ...
	 */
	public List<NotificacionDiariaBean> obtenerNotificacionesDiariasCantera(
			Long codigoUnidad, Long codigoLineaNegocio, Long codigoActividad,
			Long codigoTableroControl, Long codigoPuestoTrabajo,
			Long codigoEstado, Date fechaInicio, Date fechaFin)
			throws LogicaException {
		...
		try {
			...
			notificacionDiariaList = beanFactory
					.transformarListaNotificacionDiariaSimple(NotificacionDiariaQuerier
							.obtenerNotificacionesDiariasCantera(codigoUnidad,
									codigoLineaNegocio, codigoActividad,
									codigoPuestoTrabajo, codigoTableroControl,
	 ...
	 */
	public void insertarNotificacionPlanta(Long codigoUsuarioRegistra,
			Long codigoLineaNegocio, Long codigoTableroControl,
			Long codigoEstadoNotificacion, Date fechaRegistro,
		...
		try {
			...
			Estadonotificacion estadonotificacion = EstadoNotificacionQuerier
					.getById(codigoEstadoNotificacion);
			...
			Usuario usuarioRegistra = UsuarioQuerier
					.getById(codigoUsuarioRegistra);
			notificacionDiaria
					.setUsuarioByFkCodigoUsuarioRegistra(usuarioRegistra);

			Tablerocontrol tablerocontrol = TableroControlQuerier
					.getById(codigoTableroControl);
	 ...
	 */
	public NotificacionDiariaBean obtenerNotificacionDiariaPorCodigo(
			Long codigoNotificacionDiaria) throws LogicaException {
		if (codigoNotificacionDiaria == null) {
			mensajeError = ManejadorPropiedades
					.obtenerPropiedadPorClave(NOTIFICACION_DIARIA_CODIGO)
					+ ": "
					+ ManejadorPropiedades
							.obtenerPropiedadPorClave(FORMATO_INVALIDO);
		...
		try {
			session = PersistenciaFactory.currentSession();
			notificaciondiaria = beanFactory
					.transformarNotificacionDiariaSimple(NotificacionDiariaQuerier
	 ...
	 */
	public NotificacionDiariaBean obtenerNotificacionDiariaCantera(
			Long codigoLineaNegocio, Date fechaRegistro) throws LogicaException {
		if (codigoLineaNegocio == null) {
			mensajeError = ManejadorPropiedades
					.obtenerPropiedadPorClave(ERROR_LINEANEGOCIO_REQUERIDO);
			logger.error(mensajeError);
			throw new LogicaException(mensajeError);
		}
		...
		if (fechaRegistro == null) {
			mensajeError = ManejadorPropiedades
					.obtenerPropiedadPorClave(REPORTEECS_FECHA_REGISTRO_REQUERIDA);
			logger.error(mensajeError);
			throw new LogicaException(mensajeError);
		}
		...
		try {
			...
			notificaciondiaria = beanFactory
					.transformarNotificacionDiaria(NotificacionDiariaQuerier
							.obtenerNotificacionDiariaCanterasDespachos(
	 ...
	 */
	public NotificacionDiariaBean obtenerNotificacionDiariaCanteraBase(
			Long codigoLineaNegocio, Date fechaRegistro) throws LogicaException {
		if (codigoLineaNegocio == null) {
			mensajeError = ManejadorPropiedades
					.obtenerPropiedadPorClave(ERROR_LINEANEGOCIO_REQUERIDO);
		...
		try {
			...
			notificaciondiaria.setFechaNotificacionDiaria(notificacion
					.getFechaNotificaciondiaria());
			notificaciondiaria.setObservacionNotificacionDiaria(notificacion
					.getObservacionNotificaciondiaria());
			notificaciondiaria.setCodigo(notificacion
					.getPkCodigoNotificaciondiaria());
	 ...
	 * java.util.Date)
	 */
	public void eliminarNotificacionesProduccion(Long codigoLineaNegocio,
			Long codigoTableroControl, Date fechaRegistro,
			Long codigoNotifDiaria, Long codigoPuestoTrabajo,
			UsuarioBean usuario) throws LogicaException {
		Session session = null;
		try {
			session = PersistenciaFactory.currentSession();
			session.beginTransaction();
			NotificacionDiariaQuerier.eliminarNotificacionesProduccion(
					codigoNotifDiaria, codigoPuestoTrabajo);
			session.getTransaction().commit();
		} catch (ElementoEliminadoException e) {
			if (session != null) {
				PersistenciaFactory.rollbackTransaction(session
						.getTransaction());
			}
	 ...
	 */
	public NotificacionDiariaBean aprobarNotificacion(
			NotificacionDiariaBean notificacion, UsuarioBean usuario)
			throws LogicaException {

		if (notificacion == null || notificacion.getCodigo() == null) {
			throw new LogicaException(
					ManejadorPropiedades
							.obtenerPropiedadPorClave(NOTIFICACION_NO_SELECCIONADA));
		}
		...
		try {
			...
			boolean movimientosAutomaticos = MovimientoQuerier
					.verificarMovimientosAutomaticosEnFecha(fechaNotif);
			if (movimientosAutomaticos == false) {
				...
				registrarMovimientoDespachoProductosTerminados(fechaNotif,
						usuario);
			}
	...
	public static void mainsss(String[] args) {
		String fechaFormateada = PropiedadesSap.formatoFechaPbudat(FechaUtil
				.FormaterFecha("10/10/2015"));
		try {
			System.out.println(" 1 -->" + (new Date()));
			NotificacionDiariaLogic no = new NotificacionDiariaLogic();
			UsuarioLogic u = new UsuarioLogic();
			System.out.println(" 2 -->" + (new Date()));
			no.registrarMovimientoIngresoMateriasPrimas(
					FechaUtil.FormaterFecha("15/04/2016"),
					u.obtenerUsuario(160L));
			System.out.println(" 3 -->" + (new Date()));
			no.registrarMovimientoDespachoProductosTerminados(
					FechaUtil.FormaterFecha("15/04/2016"),
					u.obtenerUsuario(160L));
	...
	private void registrarMovimientoIngresoMateriasPrimas(Date fechaNotif,
			UsuarioBean usuario) throws LogicaException {
		SociedadBean sociedad = sociedadFacade
				.obtenerSociedadPorCodigoSAP(CODIGO_SAP_SOCIEDAD);
		...
		UnidadMedidaBean unidadMedida = unidadMedidaFacade
				.obtenerUnidadMedidaPorNombre(ManejadorPropiedades
		...
		List<String> registrosRFC = rfcPeMMIngresosDiarioFacade
				.obtenerDatosTabla(fechaFormateada);

		for (String string : registrosRFC) {
			System.out.println("REGISTRO SAP ---->" + string);
		}

		for (Iterator<String> iterator = registrosRFC.iterator(); iterator
				.hasNext();) {
			...
			ProductoBean producto = productoFacade
					.obtenerProductoPorCodigoSAP(codigoSAPProducto);

			if (producto == null) {
				String mensaje = MessageFormat
						.format(ERROR_CODIGO_PRODUCTO_SAP,
								new Object[] {
										codigoSAPProducto,
			...
			List<HojaRutaBean> hojasRuta = hojaRutaLogic
					.obtenerHojasRutaActivaPorProducto(producto.getCodigo());

			if (hojasRuta != null && hojasRuta.size() > 0) {

				String fecha = campos[4];
				fechaMovimiento = FechaUtil.convertirAFecha(fecha,
						FechaUtil.PATRON_FECHA_APLICACION, null);
				...
				documentoMaterial
						.setTipodocumentomaterial(tipodocumentomaterial);
				...
				asignarUbicacionPorDefectoDeAlmacen(movimientoBean,
						codigoSapAlmacen);
				...
				try {
					LineaNegocioBean lineaNegocio = lineaNegocioFacade
							.obtenerLineasNegocioSegunHojaruta(producto
									.getCodigo());
					movimientoBean.setLineanegocio(lineaNegocio);

					NumberFormat numberFormat = NumberFormat
							.getInstance(Locale.ENGLISH);
					cantidadTM = numberFormat.parse(campos[7].trim())
							.doubleValue();

					// AGREGADO POR FABIAN GELDRES RESUELVE EL CASI 1495
					if (campos[7].trim().indexOf(SIGNO_MENOS) >= 0
							&& campos[campos.length - 1].trim().equals(
									INGRESO_MERCADERIA)) {
	 ...
	 *            MovimientoBean movimiento al que se le asignara la ubicacion
	 * @param codigoSapAlmacen
	 *            String codigo sap del alamcen
	 * @throws LogicaException
	 *             si la busqueda del alacen o ubicion falla
	 */
	private void asignarUbicacionPorDefectoDeAlmacen(
			MovimientoBean movimientoBean, String codigoSapAlmacen)
			throws LogicaException {
		AlmacenBean almacenBean = almacenLogicFacade
				.obtenerAlmacenPorCodigoSAP(codigoSapAlmacen);
		...
		String claveAlmacen = ALMACEN_PARA_UBICACION_POR_DEFECTO
				+ almacenBean.getCodigo();

		Long codigoUbicacion = Long.parseLong(ManejadorPropiedades
				.obtenerPropiedadPorClave(claveAlmacen));
		UbicacionBean ubicacionBean = ubicacionLogicFacade
				.obtenerUbicacion(codigoUbicacion);
	 ...
	 * @throws LogicaException
	 */
	private ProductoBean obtenerProductoSegunCodigoSap(Long unidadMedida)
			throws LogicaException {
		ProductoBean producto = null;

		producto = productoFacade.obtenerProductoPorCodigoSAPUnidadMedida(
				codigoSAPProducto, unidadMedida);

		if (producto == null) {
			String mensaje = MessageFormat.format(ERROR_CODIGO_PRODUCTO_SAP,
					new Object[] { codigoSAPProducto,
	...
	private ResultadoBeanImpl exportarConsumoDeMateriasPrimas(Date fechaNotif,
			Long codigoNotif) throws LogicaException {
		...
		estadoRetornoRFC = rfcPeMMConMatPrimaFacade.ingresarDatosPorLote(
				fechaP_BUDAT, codigoNotif, fechaBLDATT);
		return estadoRetornoRFC;
	}

	private PeriodoContableBean obtenerPeriodoContable(Date fecha)
			throws LogicaException {
		String fechaString = FechaUtil.convertirDateToString(fecha);
		String[] fechaStringArray = fechaString
				.split(FechaUtil.SEPARADOR_CAMPOS_FECHA_APP);

		Short mesPeriodo = new Short(fechaStringArray[MES]);
		int anioPeriodo = Integer.parseInt(fechaStringArray[ANIO]);

		return periodoContableFacade.getPeriodoContablePorFecha(mesPeriodo,
				anioPeriodo);
	}

	public void registrarMovimientoDespachoProductosTerminados(Date fechaNotif,
			UsuarioBean usuario) throws LogicaException {
		...
		if (parametroSistemaBean == null) {
			throw new LogicaException("No se encontró el parametro de sistema "
					+ ConstantesParametro.CODIGO_UNIDAD_MEDIDA_TM);
		}

		if (parametroSistemaBean.getValor() == null) {
			throw new LogicaException(
					"No se encontró el valor del parametro de sistema "
							+ ConstantesParametro.CODIGO_UNIDAD_MEDIDA_TM);
		}
		Long codigo_unidad_medida_TM = Long.valueOf(parametroSistemaBean
				.getValor());

		String fechaFormateada = PropiedadesSap.formatoFechaPbudat(fechaNotif);
		SociedadBean sociedad = sociedadFacade
				.obtenerSociedadPorCodigoSAP(CODIGO_SAP_SOCIEDAD);
		TipoDocumentoMaterialBean tipoDocMat = tipoDocumentoFacade
				.obtenerTipoDocumentoPorNombre(TIPO_DOC_MAT_SALIDA);
		// String unidadMedidaProperty =
		// ManejadorPropiedades.obtenerPropiedadPorClave(UNIDADMEDIDA);
		UnidadMedidaBean unidadMedida = unidadMedidaFacade
				.obtenerUnidadMedidaPorCodigo(codigo_unidad_medida_TM);

		PeriodoContableBean periodo = obtenerPeriodoContable(fechaNotif);

		// Codigo estado liberada
		Long codgoEstadoLiberada = Long.parseLong(ManejadorPropiedades
				.obtenerPropiedadPorClave(ESTADO_ORDEN_PRODUCCION_LIBERADA));
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(fechaNotif);
		int mes = calendar.get(Calendar.MONTH) + 1;
		int anio = calendar.get(Calendar.YEAR);

		List<String> registrosRFC = rfcPeMMStockDespachosFacade
				.obtenerDatosTabla(fechaFormateada);
		...
		String codigosMovTraslados = ManejadorPropiedades
				.obtenerPropiedadPorClave(CODIGOS_MOVIMIENTOS_TRANSLADO);
		...
		for (String registro : registrosRFC) {
			...
			ProductoBean producto = obtenerProductoSegunCodigoSap(unidadMedida
					.getCodigo());
			if (producto == null) {
				continue;
			}
			...
			fechaMovimiento = FechaUtil.convertirAFecha(fecha,
					FechaUtil.PATRON_FECHA_RFC_DESPACHOS, null);
			...
			LineaNegocioBean lineaNegocioBean = lineaNegocioFacade
					.obtenerLineasNegocioSegunHojaruta(producto.getCodigo());
			...
			if (codigosMovTraslados.equals(tipoMovSap)) {
				if (negativo) {
					// Salida de material
					TipoMovimientoBean tipoMovimiento = tipoMovimientoFacade
							.obtenerTipoMovimiento(Long.valueOf(1));
					movimientoBean.setTipomovimiento(tipoMovimiento);
				} else {
					// Ingreso de material
					TipoMovimientoBean tipoMovimiento = tipoMovimientoFacade
							.obtenerTipoMovimiento(Long.valueOf(3));
					movimientoBean.setTipomovimiento(tipoMovimiento);
				}
				...
				TipoMovimientoBean tipoMovimiento = tipoMovimientoFacade
						.obtenerTipoMovimientoPorCodigoSAP(tipoMovSap);
				if (tipoMovimiento != null) {
					movimientoBean.setTipomovimiento(tipoMovimiento);
				} else {
					String error = MessageFormat.format(ERROR_TIPO_MOV_RFC,
							new Object[] { tipoMovSap });
					logger.error(error);
					throw new LogicaException(error);
				}
			}

			asignarMedioAlmacenamiento(codgoEstadoLiberada, mes, campos,
					producto, movimientoBean, lineaNegocioBean.getCodigo(),
					anio);
	...
	private Double obtenerCantidadTM(Double cantidad) {
		...
		Double factor = Double.parseDouble(ManejadorPropiedades
				.obtenerPropiedadPorClave(FACTOR_CONVERSION_BOLSA));

		try {
			CapacidadBolsaProductoBean bolsaProductoBean = null;
			ParametroSistemaBean parametroBean = null;
			if (codigoSAPProducto != null) {
				bolsaProductoBean = capacidadBolsaProductoLogicFacade
						.obtenerPorCodigoSap(codigoSAPProducto);
			}

			if (bolsaProductoBean != null
					&& bolsaProductoBean.getCapacidadNetaBolsa() != null) {

				factor = bolsaProductoBean.getCapacidadNetaBolsa();
			} else {
				parametroBean = parametroSistemaLogicFacade
						.obtenerParametroSistema(ConstantesParametro.CAPACIDAD_BOLSA_DEFECTO);

				if (parametroBean != null && parametroBean.getValor() != null
						&& !parametroBean.getValor().equals("")) {

					factor = NumberUtil.convertirStringToDouble(parametroBean
							.getValor());
				}
			}
		} catch (LogicaException e) {
	 ...
	 * @throws LogicaException
	 */
	private void asignarMedioAlmacenamiento(Long codgoEstadoLiberada, int mes,
			String[] campos, ProductoBean producto,
			MovimientoBean movimientoBean, Long codigoLineaNeg, Integer anio)
			throws LogicaException {
		int tamanoArrayConSilo = 8;

		if (campos.length > tamanoArrayConSilo
				&& !StringUtils.isBlank(campos[8])) {
			...
			Ordenproduccion ordenProduccion = OrdenProduccionQuerier
					.obtenerPorProductoMesYEstado(mes, producto.getCodigo(),
			...
			Long codigoProceso = ordenProduccion.getProduccion().getProceso()
					.getPkCodigoProceso();

			try {
				Medioalmacenamiento silo = MedioAlmacenamientoQuerier
						.findSiloByNumeroYProduccion(numeroMedioAlmacenamiento,
								producto.getCodigo(), codigoProceso);
				MedioAlmacenamientoBean medioAlmacenamientoBean = beanFactory
						.transformarMedioAlmacenamiento(silo);
				movimientoBean.setMedioalmacenamiento(medioAlmacenamientoBean);
			} catch (SesionVencidaException e) {
				documentoFacade.eliminarDocumentosGeneradosSap(movimientoBean
						.getFechaMovimiento());
				throw new LogicaException(e.getMensaje(), e);
			} catch (ElementoNoEncontradoException e) {
				documentoFacade.eliminarDocumentosGeneradosSap(movimientoBean
	...
	public String obtenerCodigoSapTransformado(String codigoSAP) {
		...
		String codigoSap8Dig = codigoSAP.substring(codigoSAP.length() - 8,
				codigoSAP.length());
		String resultado = codigoSap8Dig.substring(0, 3) + "-"
				+ codigoSap8Dig.substring(3, codigoSap8Dig.length());

		return resultado;
	}

	private void cambiarAEstadoAprobado(NotificacionDiariaBean notificacion)
			throws LogicaException {
		try {
			Long codigoEstadoAprobado = new Long(
					ManejadorPropiedades
			...
			mensajeError = ManejadorPropiedades
					.obtenerPropiedadPorClave(NOTIFICACIONPLANTA_ESTADO_NO_CAMBIADO);
			throw new LogicaException(mensajeError, e);
		}
	}
	 ...
	 */
	public NotificacionDiariaBean obtenerNotificacion(Long codigoLineaNegocio,
			Date fechaRegistro) throws LogicaException {
		if (codigoLineaNegocio == null) {
			mensajeError = ManejadorPropiedades
					.obtenerPropiedadPorClave(ERROR_LINEANEGOCIO_REQUERIDO);
		...
		try {
			...
			notificaciondiaria = beanFactory
					.transformarNotificacionDiaria(NotificacionDiariaQuerier
							.obtenerNotificacion(codigoLineaNegocio,
									fechaRegistro));
			return notificaciondiaria;
		} catch (ElementoNoEncontradoException e) {
			mensajeError = ManejadorPropiedades
	...
	public boolean desaprobarNotificacion(Long lineaNegocio, Date fecha) {
		...
		try {
			...
			List<Tablakardex> tablakardexes = TablaKardexQuerier
					.obtenerTablaKardexPorPeriodoParteDiario(lineaNegocio,
							fecha);
			...
			TablaKardexQuerier
					.eliminarKardexAlmacenamientoPorCodigosTablaKardex(codigos);

			// Eliminar valorpromvariablecalidad
			ValorPromVariableCalidadQuerier
					.eliminarPorCodigoTablaKardex(codigos);

			// Eliminar Tabla Kardex
			// FABIAN FALTA ESTO DE ELIMINAR PERO ANTES TIENES QUE ACTUALIZAR
			// LOS SALDO DE PRODUCCION DIARIA

			ProduccionDiariaQuerier
					.actualizarProduccionDiarioConValoresKardex(tablakardexes);
			// ELIMINAR KARDEX
			TablaKardexQuerier.eliminarPorcodigos(codigos);

			// Obtener Codigos ConsumoPuesto de Trabajo

			codigos = ConsumoPuestoTrabajoQuerier
					.obtenerCodigosPorLineaNegYFecha(lineaNegocio, fecha);

			// Eliminar factorconsumopuestotrabajo
			FactorconsumopuestotrabajoQuerier
					.eliminarPorCodigodConsumoPuestoTrabajo(codigos);
			// Eliminar factorvariacionpuestotrabajo

			FactorVariacionPuestoTrabajoQuerier
					.eliminarPorCodigodConsumoPuestoTrabajo(codigos);
			...
			codigos = ProductoGeneradoQuerier.obtenerCodigosPorLineNegFecha(
					lineaNegocio, fecha);

			FactorVariacionProduccionPuestoTrabajoQuerier
					.eliminarPorCodigoProductoGenerado(codigos);
			...
			DocumentoMaterialQuerier
					.eliminarPorFechaYMovimientoSAP(fecha, true);

			// Eliminacion de PARTE DIARIO Y
			if (FechaUtil.obtenerDiaFecha(fecha).equals("01")) {

				ProduccionPuestoTrabajoQuerier.eliminarFechaLineaNegocio(
						lineaNegocio, fecha);

				ProduccionDiariaQuerier.eliminarSegunFechaLineaNeg(
						lineaNegocio, fecha);
				ParteDiarioQuerier.eliminarFechaLineaNegocio(lineaNegocio,
						fecha);

			}

			Notificaciondiaria notificacion = NotificacionDiariaQuerier
					.obtenerNotificacion(lineaNegocio, fecha);
	...
	public static void main3(String[] args) {
		...
		UsuarioLogic u = new UsuarioLogic();
		try {
			n.cerrarNotificacion(n.obtenerNotificacionDiaria(1552L),
					u.obtenerUsuario(11L));
		} catch (LogicaException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public ResultadoBeanImpl cerrarNotificacion(
			NotificacionDiariaBean notificacion, UsuarioBean usuario)
			throws LogicaException {
		...
		if (notificacion == null || notificacion.getCodigo() == null) {
			throw new LogicaException(
					ManejadorPropiedades
							.obtenerPropiedadPorClave(NOTIFICACION_NO_SELECCIONADA_CERRAR));
		}
		...
		if (notificacion != null
				&& notificacion.getEstadoNotificacion()
						.getNombreEstadoNotificacion().equals(ESTADO_CERRADO)) {
		...
		NotificacionDiariaBean notificacionBean = this
				.obtenerPrimeraNotificacionDiariaAprobada(notificacion);
		if (notificacionBean != null) {
			if (!FechaUtil.comparaFechasSoloDiaMesYAnio(
					notificacion.getFechaNotificacionDiaria(),
					notificacionBean.getFechaNotificacionDiaria())) {
				if (notificacionBean.getEstadoNotificacion()
						.getNombreEstadoNotificacion().equals(ESTADO_APROBADO)) {
					mensajeError = MessageFormat.format(
							ERROR_NOTIFICACION_CIERRE,
							new Object[] { notificacionBean
									.getFechaNotificacionDiaria() });
					throw new LogicaException(mensajeError);
				}
			}
		...
		if (notificacionBean != null
				&& !FechaUtil.comparaFechasSoloDiaMesYAnio(notificacion
						.getFechaNotificacionDiaria(), FechaUtil
						.obtenerFechaDiaSiguiente(notificacionBean
								.getFechaNotificacionDiaria()))) {
			mensajeError = MessageFormat.format(ERROR_NOTIFICACION_CIERRE,
					new Object[] { FechaUtil
							.obtenerFechaDiaSiguiente(notificacionBean
									.getFechaNotificacionDiaria()) });
			throw new LogicaException(mensajeError);
		}
		...
		Integer annio;
		Long codigoEstadoCerrado = Long.valueOf(ManejadorPropiedades
				.obtenerPropiedadPorClave(CODIGO_ESTADONOTIFICACION_CERRADO));
		try {

			session = PersistenciaFactory.currentSession();
			tx = session.beginTransaction();

			Notificaciondiaria notificacionDiaria = NotificacionDiariaQuerier
					.getById(codigoNotif);
			if (activacionExportarConsumoMateriaPrimas()) {

				estadoProcesoRfc = exportarConsumoDeMateriasPrimas(
						notificacionDiaria.getFechaNotificaciondiaria(),
						codigoNotif);

				if (estadoProcesoRfc.getExitoOperacion()) {
					// Cambio de Estado para la notificacion
					Estadonotificacion estadoCerrado = EstadoNotificacionQuerier
							.getById(codigoEstadoCerrado);
					notificacionDiaria.setEstadonotificacion(estadoCerrado);
					notificacionDiaria
							.setUsuarioByFkCodigoUsuarioCierra(beanFactory
									.transformarUsuarioBean(usuario));
					NotificacionDiariaQuerier.update(notificacionDiaria);

					// Cerrar parte diario al ultima notificacion
					Short mes = (short) FechaUtil.obtenerMesFecha(notificacion
							.getFechaNotificacionDiaria());
					annio = FechaUtil.obtenerAnnioFecha(notificacion
							.getFechaNotificacionDiaria());

					if (notificacion != null
							&& FechaUtil
									.comparaFechasSoloDiaMesYAnio(
											notificacion
													.getFechaNotificacionDiaria(),
											FechaUtil.getUltimoDiaMes(
													(short) (mes - 1), annio)
													.getTime())) {
						Estadopartediario estadoPd = EstadoParteDiarioQuerier
								.getById(Long
										.valueOf(CODIGO_ESTADO_PARTEDIARIO_CERRADO));
						Partediario parteDiario = ParteDiarioQuerier
								.getByPeriodoContableYLineaNegocio(mes, annio,
										notificacion.getLineaNegocio()
												.getCodigo());
	...
	public boolean reprocesarNotificacion(
			NotificacionDiariaBean notificacionBean, UsuarioBean usuario)
			throws LogicaException {
		Transaction tx = null;
		Session session = null;
		boolean condicion = false;
		try {
			...
			if (notificacionBean.getEstadoNotificacion()
					.getNombreEstadoNotificacion().equals(ESTADO_CERRADO)) {
				mensajeError = MessageFormat.format(
						ERROR_NOTIFICACION_CERRADO_REPROCESAR,
						new Object[] { notificacionBean
								.getFechaNotificacionDiaria() });
				throw new LogicaException(mensajeError);
			}
			...
			// obtenerUltimaNotificacionDiariaAprobada
			NotificacionDiariaBean notificacionBeanAprobado = this
					.obtenerUltimaNotificacionDiariaAprobada(notificacionBean);

			if (!FechaUtil.comparaFechasSoloDiaMesYAnio(
					notificacionBean.getFechaNotificacionDiaria(),
					notificacionBeanAprobado.getFechaNotificacionDiaria())) {
				mensajeError = MessageFormat.format(
						ERROR_NOTIFICACION_REPROCESO,
						new Object[] { notificacionBeanAprobado
								.getFechaNotificacionDiaria() });
				throw new LogicaException(mensajeError);
			...
			Short mes = Short.valueOf(FechaUtil
					.obtenerMesFecha(notificacionBean
							.getFechaNotificacionDiaria())
					+ "");
			Integer annio = FechaUtil.obtenerAnnioFecha(notificacionBean
					.getFechaNotificacionDiaria());
			Long lineaNegocio = notificacionBeanAprobado.getLineaNegocio()
					.getCodigo();

			Ajusteproduccion ajuste = AjusteProduccionQuerier
					.getByLineaNegocioMesYAno(mes, annio, lineaNegocio);

			if (ajuste != null) {
				if (!ajuste.getEstadoajusteproduccion()
						.getNombreEstadoajusteproduccion().equals("Inicial")) {
	...
	public boolean abrirNotificacionDiaria(Long codigo, UsuarioBean usuario)
			throws LogicaException {
		Transaction tx = null;
		Session session = null;
